 ...
 * <br>
 * Raid Bosses (RB) are mobs which are supposed to be defeated by a party of
 * several players. It extends most of {@link Monster} aspects.<br>
 * They automatically teleport if out of their initial spawn area, and can
 * randomly attack a Player from their Hate List once attacked.
 */
			...
			if (!isDead()) {
				...
				if (!isInCombat()) {
					...
					if (getNpcId() != 29095 && Rnd.nextBoolean()) {
						...
						if (!isInsideRadius(spawn.getLocX(), spawn.getLocY(), spawn.getLocZ(),
								Math.max(Config.MAX_DRIFT_RANGE, 200), true, false))
		...
		if (killer != null) {
			...
			if (player != null) {
				...
				if (!Config.BOSS_DEFEATED_BY_CLAN_MEMBER_MSG.isEmpty() && !Config.BOSS_DEFEATED_BY_PLAYER_MSG.isEmpty())
					World.announceToOnlinePlayers(player.getClan() != null
							? Config.BOSS_DEFEATED_BY_CLAN_MEMBER_MSG.replace("%raidboss%", getName())
									.replace("%player%", killer.getName()).replace("%clan%", player.getClan().getName())
							: Config.BOSS_DEFEATED_BY_PLAYER_MSG.replace("%raidboss%", getName()).replace("%player%",
									killer.getName()),
							true);
			}
		}
