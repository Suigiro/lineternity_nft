...
public class AuctionManager extends Folk {
	...
	public void onBypassFeedback(Player player, String command) {
		if (command.startsWith("auction")) {
			...
			if (player.getInventory().getItemByItemId(item.getCostId()) == null
					|| player.getInventory().getItemByItemId(item.getCostId()).getCount() < item.getCostCount()) {
			...
			AuctionTable.getInstance()
					.addItem(new AuctionItem(AuctionTable.getInstance().getNextAuctionId(), player.getObjectId(),
							player.getInventory().getItemByObjectId(itemId).getItemId(), itemAmount,
							player.getInventory().getItemByObjectId(itemId).getEnchantLevel(), costId, costCount));
	...
	private void showMyItems(Player player, int page) {
		...
		for (AuctionItem item : items.get(page)) {
			html += "<tr>";
			html += "<td><img src=\"" + ItemTable.getInstance().getTemplate(item.getItemId()).getIcon()
					+ "\" width=32 height=32 align=center></td>";
			html += "<td>Item: " + (item.getEnchant() > 0
					? "+" + item.getEnchant() + " " + ItemTable.getInstance().getTemplate(item.getItemId()).getName()
							+ " - " + item.getCount()
					: ItemTable.getInstance().getTemplate(item.getItemId()).getName() + " - " + item.getCount());
			html += "<br1>Cost: " + item.getCostCount() + " "
					+ ItemTable.getInstance().getTemplate(item.getCostId()).getName();
			html += "</td>";
			html += "<td fixwidth=71><button value=\"Remove\" action=\"bypass -h npc_" + getObjectId() + "_remove "
					+ item.getAuctionId()
	...
	private void showAddPanel2(Player player, int itemId) {
		...
		String html = "";
		html += "<html><title>Auction Shop</title><body><center><br1>";
		html += "<img src=\"" + ItemTable.getInstance().getTemplate(item.getItemId()).getIcon()
				+ "\" width=32 height=32 align=center>";
		html += "Item: "
				+ (item.getEnchantLevel() > 0 ? "+" + item.getEnchantLevel() + " " + item.getName() : item.getName());
		...
		html += "<br><button value=\"Add item\" action=\"bypass -h npc_" + getObjectId() + "_addit2 " + itemId
				+ " $ebox $count " + (item.isStackable() ? "$amm" : "1")
				+ "\" width=70 height=21 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\">";
		html += "</center></body></html>";
	...
	private void showAddPanel(Player player, int page) {
		...
		for (ItemInstance item : items.get(page)) {
			...
			html += "<img src=\"" + ItemTable.getInstance().getTemplate(item.getItemId()).getIcon()
					+ "\" width=32 height=32 align=center></td>";
			html += "<td>" + (item.getEnchantLevel() > 0 ? "+" + item.getEnchantLevel() + " " + item.getName()
					: item.getName());
			html += "</td>";
	...
	private void showAuction(Player player, int page, String search) {
		...
		for (AuctionItem item : AuctionTable.getInstance().getItems()) {
			if (item.getOwnerId() != player.getObjectId() && (!src
					|| (src && ItemTable.getInstance().getTemplate(item.getItemId()).getName().contains(search)))) {
				temp.add(item);
		...
		html += "<button value=\"Search\" action=\"bypass -h npc_" + getObjectId()
				+ "_auction 1 - $srch\" width=70 height=21 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\">";
		...
		for (AuctionItem item : items.get(page)) {
			...
			html += "<br1>Cost: " + item.getCostCount() + " "
					+ ItemTable.getInstance().getTemplate(item.getCostId()).getName();
			html += "</td>";
			html += "<td fixwidth=71><button value=\"Buy\" action=\"bypass -h npc_" + getObjectId() + "_buy "
					+ item.getAuctionId()
					+ "\" width=70 height=21 back=\"L2UI.DefaultButton_click\" fore=\"L2UI.DefaultButton\">";
			html += "</td></tr>";
		}
		...
		if (items.keySet().size() > 1) {
			if (page > 1)
				html += "<a action=\"bypass -h npc_" + getObjectId() + "_auction " + (page - 1) + " - " + search
						+ "\"><- Prev</a>";
