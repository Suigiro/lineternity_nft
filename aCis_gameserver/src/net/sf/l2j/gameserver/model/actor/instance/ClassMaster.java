 ...
 * <br>
 * You can customize class rewards as needed items. Check npc.properties for
 ...
 * NOTE : you have to take 1st class then 2nd, if you try to take 2nd directly
 * it won't work.</li>
 * <li>the "allow_entire_tree" version, where you can take class depending of
 * your current path.<br>
 * NOTE : you don't need to be of the good level.</li>
 * </ul>
 * Added to the "change class" function, this NPC can noblesse and give
 * available skills (related to your current class and level).
 */
...
//			return;
	...
	private static final void showTutorialHtml(Player player) {
		...
		for (ClassId cid : ClassId.values()) {
			if (validateClassId(currentClassId, cid))
				StringUtil.append(menu, "<a action=\"link CO", String.valueOf(cid.getId()), "\">",
						PlayerData.getInstance().getClassNameById(cid.getId()), "</a><br>");
		}
			...
			if (currentClassId.level() >= level)
				...
				if (player.getLevel() >= minLevel || Config.ALLOW_ENTIRE_TREE) {
					...
					for (ClassId cid : ClassId.VALUES) {
						...
						if (validateClassId(currentClassId, cid))
							StringUtil.append(menu, "<a action=\"bypass -h npc_%objectId%_change_class ", cid.getId(),
									"\">", PlayerData.getInstance().getClassNameById(cid.getId()), "</a><br>");
					}
	...
	private static String getRequiredItems(int level) {
		...
		for (IntIntHolder item : neededItems)
			StringUtil.append(sb, "<tr><td><font color=\"LEVEL\">", item.getValue(), "</font></td><td>",
					ItemTable.getInstance().getTemplate(item.getId()).getName(), "</td></tr>");
