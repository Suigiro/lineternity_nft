...
/**
 * The abstract layer for an Olympiad game (individual, class and non-class
 * based).
 */
public abstract class AbstractOlympiadGame {
	...
	/**
	 * Delete all effects related to {@link Player}s, and fully heal them. Unsummon
	 * their {@link Pet} if existing.
	 */
	 ...
	 * @return true if both {@link Participant}s have been successfully teleported
	 *         on the given {@link Location}s, false otherwise.
	 */
	protected abstract boolean portPlayersToArena(List<Location> spawns);

	/**
	 * Cancel all {@link Player}s animations, set their Intention to IDLE. Affects
	 * also their {@link Summon}, if existing. Heal Players, start their HP/MP
	 * regen.
	 */
	...
	/**
	 * Reset {@link Player}s status back to regular behaviour : stop all charges and
	 * effects, add back clan & hero skills.
	 */
	...
	/**
	 * @return true if the battle is still continuing due to regular Olympiad rules
	 *         (no disconnection, correct number of Participants,...)
	 */
	...
	/**
	 * @return the divider used by that type of game to calculate Olympiad points
	 *         gain.
	 */
	...
	protected final void addPointsToParticipant(Participant par, int points) {
		...
		broadcastPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_HAS_GAINED_S2_OLYMPIAD_POINTS)
				.addString(par.getName()).addNumber(points));
	}

	protected final void removePointsFromParticipant(Participant par, int points) {
		par.updateStat(POINTS, -points);

		broadcastPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_HAS_LOST_S2_OLYMPIAD_POINTS)
				.addString(par.getName()).addNumber(points));
	}

	/**
	 * @param player : The Player to check.
	 * @return null if the tested {@link Player} passed all checks or broadcast the
	 *         {@link SystemMessage} associated to opponent defection.
	 */
	protected static SystemMessage checkDefection(Player player) {
		...
		if (player.isSubClassActive()) {
			player.sendPacket(
					SystemMessageId.SINCE_YOU_HAVE_CHANGED_YOUR_CLASS_INTO_A_SUB_JOB_YOU_CANNOT_PARTICIPATE_IN_THE_OLYMPIAD);
			return SystemMessage.getSystemMessage(
					SystemMessageId.THE_GAME_HAS_BEEN_CANCELLED_BECAUSE_THE_OTHER_PARTY_DOES_NOT_MEET_THE_REQUIREMENTS_FOR_JOINING_THE_GAME);
		}
	 ...
	 * @return true if the {@link Participant} has been successfully teleported on
	 *         the given {@link Location}, false otherwise.
	 */
	...
	/**
	 * Delete all effects related to a {@link Player}, and fully heal him. Unsummon
	 * his {@link Pet} if existing.
	 * 
	 * @param player      : The Player to affect.
	 * @param removeParty : If true, expels the Player from its current
	 *                    {@link Party}.
	 */
	...
	protected static final void buffPlayer(Player player) {
		...
		if (!player.isMageClass()) {
			...
			if (skill != null) {
				skill.getEffects(player, player);
				player.sendPacket(
						SystemMessage.getSystemMessage(SystemMessageId.YOU_FEEL_S1_EFFECT).addSkillName(1086));
			}
		} else {
			...
			if (skill != null) {
				skill.getEffects(player, player);
				player.sendPacket(
						SystemMessage.getSystemMessage(SystemMessageId.YOU_FEEL_S1_EFFECT).addSkillName(1085));
			}
		}
	}
	...
	/**
	 * Cancel all {@link Player} animations, set the Intention to IDLE. Affects also
	 * the {@link Summon}, if existing. Heal the Player, start his HP/MP regen.
	 * 
	 * @param player : The Player to affect.
	 */
	...
	/**
	 * Reset {@link Player} status back to regular behaviour : stop all charges and
	 * effects, add back clan & hero skills.
	 * 
	 * @param player : The Player to affect.
	 */
	...
	public static final void rewardParticipant(Player player, IntIntHolder[] reward) {
		...
		for (IntIntHolder it : reward) {
			final ItemInstance item = player.getInventory().addItem("Olympiad", it.getId(), it.getValue(), player,
					null);
			if (item == null)
				continue;

			iu.addModifiedItem(item);
			player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_S2_S1_S).addItemName(it.getId())
					.addNumber(it.getValue()));
		}
