...
abstract public class OlympiadGameNormal extends AbstractOlympiadGame {
	...
	protected void validateWinner(OlympiadStadiumZone stadium) {
		...
		if (_pOneCrash || _pTwoCrash) {
			if (_pTwoCrash && !_pOneCrash) {
				stadium.broadcastPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_HAS_WON_THE_GAME)
						.addString(_playerOne.getName()));
		...
		if ((_playerOne.getPlayer() == null || !_playerOne.getPlayer().isOnline())
				&& (_playerTwo.getPlayer() == null || !_playerTwo.getPlayer().isOnline())) {
			_playerOne.updateStat(COMP_DRAWN, 1);
			_playerTwo.updateStat(COMP_DRAWN, 1);
			stadium.broadcastPacket(SystemMessage.getSystemMessage(SystemMessageId.THE_GAME_ENDED_IN_A_TIE));
		} else if (_playerTwo.getPlayer() == null || !_playerTwo.getPlayer().isOnline()
			...
			rewardParticipant(_playerOne.getPlayer(), getReward());
		} else if (_playerOne.getPlayer() == null || !_playerOne.getPlayer().isOnline()
	...
	public final String[] getPlayerNames() {
		return new String[] { _playerOne.getName(), _playerTwo.getName() };
	}
	...
	protected static final void saveResults(Participant one, Participant two, int _winner, long _startTime,
			long _fightTime, OlympiadType type) {
