...
public class Olympiad {
	...
	private static final String OLYMPIAD_GET_HEROS = "SELECT olympiad_nobles.char_id, characters.char_name FROM olympiad_nobles, characters WHERE characters.obj_Id = olympiad_nobles.char_id AND olympiad_nobles.class_id = ? AND olympiad_nobles.competitions_done >= "
	...
	private static final String GET_ALL_CLASSIFIED_NOBLESS = "SELECT char_id from olympiad_nobles_eom WHERE competitions_done >= "
			+ Config.ALT_OLY_MIN_MATCHES
			+ " ORDER BY olympiad_points DESC, competitions_done DESC, competitions_won DESC";
	private static final String GET_EACH_CLASS_LEADER = "SELECT characters.char_name from olympiad_nobles_eom, characters WHERE characters.obj_Id = olympiad_nobles_eom.char_id AND olympiad_nobles_eom.class_id = ? AND olympiad_nobles_eom.competitions_done >= "
			+ Config.ALT_OLY_MIN_MATCHES
	...
	protected class OlympiadEndTask implements Runnable {
		@Override
		public void run() {
			World.toAllOnlinePlayers(SystemMessage.getSystemMessage(SystemMessageId.OLYMPIAD_PERIOD_S1_HAS_ENDED)
	...
	private void updateCompStatus() {
		synchronized (this) {
			...
			LOGGER.info("Olympiad competition period starts in {} days, {} hours and {} mins.", numDays, numHours,
					numMins);
	...
	protected void setNewOlympiadEnd() {
		World.toAllOnlinePlayers(SystemMessage.getSystemMessage(SystemMessageId.OLYMPIAD_PERIOD_S1_HAS_STARTED)
				.addNumber(_currentCycle));
	...
	public void setOlympiadEnd(Player player) {
		...
		player.sendPacket(new CreatureSay(0, Say2.TRADE, "Olympiad Manager ",
				"Olympiad period ends in " + numDays + " days, " + numHours + " hours and " + numMins + " mins."));
	}
	...
	public int getNoblessePasses(Player player, boolean clear) {
		...
		int points = (player.isHero() || HeroManager.getInstance().isInactiveHero(player.getObjectId()))
				? Config.ALT_OLY_HERO_POINTS
				: 0;
		switch (rank) {
		case 1:
			...
			for (OlympiadGameTask task : _tasks) {
				...
				if (game.getType() == OlympiadType.NON_CLASSED)
					announcement = "Olympiad class-free individual match is going to begin in Arena "
							+ (game.getStadiumId() + 1) + " in a moment.";
				else
				...
				for (OlympiadManagerNpc manager : OlympiadManagerNpc.getInstances())
					manager.broadcastPacket(
							new NpcSay(manager.getObjectId(), Say2.SHOUT, manager.getNpcId(), announcement));
			}
		}
	}
