 ...
 * previous {@link ItemInstance} enchantment.
 */
public class PreparedEntry extends Entry {
	private int _taxAmount = 0;

	public PreparedEntry(Entry template, ItemInstance item, boolean applyTaxes, boolean maintainEnchantment,
			double taxRate) {
		int adenaAmount = 0;
		...
		for (Ingredient ing : template.getIngredients()) {
			if (ing.getItemId() == 57) {
				...
				if (ing.isTaxIngredient()) {
					// if taxes are to be applied, modify/add the adena count based on the template
					// adena/ancient adena count
					if (applyTaxes)
						_taxAmount += Math.round(ing.getItemCount() * taxRate);
				} else
					adenaAmount += ing.getItemCount();

				// do not yet add this adena amount to the list as non-taxIngredient adena might
				// be entered later (order not guaranteed)
				continue;
			}
