...
public final class RequestExEnchantSkill extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		{
			if (folk == null || !player.isInsideRadius(folk, 150, false, false))
				return;
		}
		...
		if (Config.ES_SP_BOOK_NEEDED && esn.getItem() != null && !player.destroyItemByItemId("SkillEnchant",
				esn.getItem().getId(), esn.getItem().getValue(), folk, true)) {
		...
		if (Rnd.get(100) <= esn.getEnchantRate(player.getLevel())) {
			...
			player.sendPacket(
					SystemMessage.getSystemMessage(SystemMessageId.YOU_HAVE_SUCCEEDED_IN_ENCHANTING_THE_SKILL_S1)
							.addSkillName(_skillId, _skillLevel));
		}
//		if (player.isCubicBypass()) {
//			String command = "cubic_enchant_skill";
//			Jogador.useCubicCommandHandler(player, "cubic_enchant_skill");
//		} else {
