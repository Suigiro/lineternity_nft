...
public class MultiSellChoose extends L2GameClientPacket {
	...
	public void runImpl() {
		...
		if ((folk != null && !list.isNpcAllowed(folk.getNpcId()))
				|| (folk == null && list.isNpcOnly() && !player.isCubicBypass())) {
			player.setMultiSell(null);
			return;
		}
		...
		final Entry entry = list.getEntries().get(_entryId - 1); // Entry Id begins from 1. We currently use entry IDs
																	// as index pointer.
		if (entry == null) {
			player.setMultiSell(null);
			return;
		...
		// Generate a list of distinct ingredients and counts in order to check if the
		// correct item-counts are possessed by the player
		...
		for (Ingredient e : entry.getIngredients()) {
			...
			// at this point, the template has already been modified so that enchantments
			// are properly included
			// whenever they need to be applied. Uniqueness of items is thus judged by item
			// id AND enchantment level
			for (int i = ingredientsList.size(); --i >= 0;) {
				Ingredient ex = ingredientsList.get(i);

				// if the item was already added in the list, merely increment the count
				// this happens if 1 list entry has the same ingredient twice (example 2 swords
				// = 1 dual)
			...
			// if it's a new ingredient, just store its info directly (item id, count,
			// enchantment)
			if (newIng)
				ingredientsList.add(e);
		}

		// now check if the player has sufficient items in the inventory to cover the
		// ingredients' expences
		for (Ingredient e : ingredientsList) {
			...
			if (e.getItemId() == CLAN_REPUTATION) {
				...
				// otherwise, check only the count of items with exactly the needed enchantment
				// level
				if (inv.getInventoryItemCount(e.getItemId(), list.getMaintainEnchantment() ? e.getEnchantLevel() : -1,
						false) < ((Config.ALT_BLACKSMITH_USE_RECIPES || !e.getMaintainIngredient())
		...
		for (Ingredient e : entry.getIngredients()) {
			if (e.getItemId() == CLAN_REPUTATION) {
				...
				player.sendPacket(
						SystemMessage.getSystemMessage(SystemMessageId.S1_DEDUCTED_FROM_CLAN_REP).addNumber(amount));
				...
				if (Config.ALT_BLACKSMITH_USE_RECIPES || !e.getMaintainIngredient()) {
					// if it's a stackable item, just reduce the amount from the first (only)
					// instance that is found in the inventory
					if (itemToTake.isStackable()) {
						if (!player.destroyItem("Multisell", itemToTake.getObjectId(), (e.getItemCount() * _amount),
						...
						// a) list maintains enchantment: get the instances that exactly match the
						// requested enchantment level
						// b) list does not maintain enchantment: get the instances with the LOWEST
						// enchantment level

						// a) if enchantment is maintained, then get a list of items that exactly match
						// this enchantment
						...
						// b) enchantment is not maintained. Get the instances with the LOWEST
						// enchantment level
						{
							for (int i = 1; i <= (e.getItemCount() * _amount); i++) {
								...
								// get item with the LOWEST enchantment level from the inventory (0 is the
								// lowest)
								if (itemToTake.getEnchantLevel() > 0) {
									for (ItemInstance item : inventoryContents) {
										if (item.getEnchantLevel() < itemToTake.getEnchantLevel()) {
											...
											// nothing will have enchantment less than 0. If a zero-enchanted item is
											// found, just take it
											if (itemToTake.getEnchantLevel() == 0)
												break;
										}
								...
								if (!player.destroyItem("Multisell", itemToTake.getObjectId(), 1, player.getTarget(),
										true)) {
									player.setMultiSell(null);
									return;
								}
		...
		for (Ingredient e : entry.getProducts()) {
			if (e.getItemId() == CLAN_REPUTATION)
				...
				if (e.getItemCount() * _amount > 1)
					sm = SystemMessage.getSystemMessage(SystemMessageId.EARNED_S2_S1_S).addItemName(e.getItemId())
							.addNumber(e.getItemCount() * _amount);
				else {
					if (list.getMaintainEnchantment() && e.getEnchantLevel() > 0)
						sm = SystemMessage.getSystemMessage(SystemMessageId.ACQUIRED_S1_S2)
								.addNumber(e.getEnchantLevel()).addItemName(e.getItemId());
					else
