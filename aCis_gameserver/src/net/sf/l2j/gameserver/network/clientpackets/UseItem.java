...
public final class UseItem extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		if (activeChar.isAlikeDead() || activeChar.isStunned() || activeChar.isSleeping() || activeChar.isParalyzed()
				|| activeChar.isAfraid())
			return;
		...
		if (!Config.KARMA_PLAYER_CAN_TELEPORT && activeChar.getKarma() > 0) {
			...
			if (sHolders != null) {
				for (IntIntHolder sHolder : sHolders) {
					...
					if (skill != null && (skill.getSkillType() == L2SkillType.TELEPORT
							|| skill.getSkillType() == L2SkillType.RECALL))
						return;
				}
			}
		}
		...
		/*
		 * The player can't use pet items if no pet is currently summoned. If a pet is
		 * summoned and player uses the item directly, it will be used by the pet.
		 */
		if (item.isPetItem()) {
			...
			if (item.isEquipped()) {
				...
				activeChar
						.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.PET_TOOK_OFF_S1).addItemName(item));
			} else {
		...
		if (item.isEquipable()) {
			...
			if (activeChar.getAttackType() == WeaponType.FISHINGROD
					&& item.getItem().getItemType() == EtcItemType.LURE) {
			...
			Jogador.useItemHandler(activeChar, item);
