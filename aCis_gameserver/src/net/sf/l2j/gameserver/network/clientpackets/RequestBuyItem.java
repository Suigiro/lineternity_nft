...
public final class RequestBuyItem extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		// merchant = (Npc) target;
		//
		// if (merchant == null || !buyList.isNpcAllowed(merchant.getNpcId()) ||
		...
		if (!player.isCubicBypass()) {
			merchant = (player.getTarget() instanceof Merchant) ? (Folk) player.getTarget() : null;
			if (merchant == null || !merchant.canInteract(player))
				return;
		}

//		if (buyList == null) {
//			Utils.handleIllegalPlayerAction(player, player.getName() + " of account " + player.getAccountName()
//					+ " sent a false BuyList list_id " + this._listId, Config.PUNISHMENT);
//			return;
//		}
		...
		if (merchant != null) {
			...
			if (!htmlFolder.isEmpty()) {
				final String content = HtmCache.getInstance()
						.getHtm("data/html/" + htmlFolder + "/" + merchant.getNpcId() + "-bought.htm");
				if (content != null) {
