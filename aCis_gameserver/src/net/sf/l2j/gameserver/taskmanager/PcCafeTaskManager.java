...
public class PcCafeTaskManager implements Runnable {
    ...
    public void run() {
        World.getInstance().getPlayers().stream()
                .filter(player -> player.getLevel() >= Config.PCB_MIN_LEVEL && !player.getClient().isDetached())
                .forEach(player -> {
                    if (!player.isAFK())
                        player.increasePcCafePoints(Rnd.get(Config.PCB_POINT_MIN, Config.PCB_POINT_MAX),
                                Rnd.get(100) <= Config.PCB_CHANCE_DUAL_POINT);
                });
    }
