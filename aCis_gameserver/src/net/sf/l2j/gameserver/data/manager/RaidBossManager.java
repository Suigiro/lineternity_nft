...
/**
 * Loads and store {@link RaidBoss}es informations, using {@link BossSpawn}
 * holder.
 */
public class RaidBossManager {
	...
	private void load() {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			...
			while (rs.next()) {
				...
				addNewSpawn(spawn, rs.getLong("respawn_time"), rs.getDouble("currentHP"), rs.getDouble("currentMP"),
						false);
			}
		} catch (Exception e) {
	...
	/**
	 * @param id : The id used as reference.
	 * @return the {@link BossStatus} of an existing {@link BossSpawn}, based on a
	 *         id.
	 */
	...
	/**
	 * Retrieve an existing {@link BossSpawn} based on a {@link RaidBoss} id, and
	 * activate it's {@link BossSpawn#onDeath()} method.
	 * 
	...
	/**
	 * Add a new {@link BossSpawn}, based on a {@link L2Spawn}. It is both used on
	 * server startup and admincommand.<br>
	 * <br>
	 * Database is either refreshed using forceSave boolean flag, or if a difference
	 * of status has been detected.
	 * 
	 ...
	 * @param forceSave   : If true, we force the insertion of this spawn into the
	 *                    database.
	 */
	...
	/**
	 * Delete an existing {@link BossSpawn} based on a {@link L2Spawn}. Drop it from
	 * {@link SpawnTable}.
	 * 
	 * @param spawn : The spawn used as reference.
	 */
	...
	/**
	 * Cleanup data. Cancel all running tasks, save hp/mp and location if boolean
	 * flag is true, delete existing {@link BossSpawn} entries.
	 * 
