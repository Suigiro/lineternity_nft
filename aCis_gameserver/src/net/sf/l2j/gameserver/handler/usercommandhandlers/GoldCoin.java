...
public class GoldCoin implements IUserCommandHandler {
	private static final int[] COMMAND_IDS = { 114, 115 };
		...
		if (id == 114) {
			for (IntIntHolder gold : Config.BANKING_SYSTEM_GOLDCOIN) {
				if (activeChar.getInventory().getInventoryItemCount(57, 0) >= Config.BANKING_SYSTEM_ADENA) {
					...
					activeChar.sendMessage("You don't have enough Adena to convert to "
							+ ItemTable.getInstance().getTemplate(gold.getId()).getName() + "(s), You need to "
			...
			for (IntIntHolder gold : Config.BANKING_SYSTEM_GOLDCOIN) {
				if (activeChar.getInventory().getInventoryItemCount(gold.getId(), 0) >= gold.getValue()) {
					...
					activeChar
							.sendMessage("You have none " + ItemTable.getInstance().getTemplate(gold.getId()).getName()
									+ " to exchange for " + Config.BANKING_SYSTEM_ADENA + " Adena.");
			}
		}
