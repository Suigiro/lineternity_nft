...
public class AdminTeleport implements IAdminCommandHandler {
	private static final String[] ADMIN_COMMANDS = { "admin_runmod", "admin_instant_move", "admin_tele",
			"admin_tele_areas", "admin_goto", "admin_teleportto", // deprecated
			"admin_recall", "admin_recall_all", "admin_recall_party", "admin_recall_clan", "admin_move_to",
			"admin_send_home" };
...
//					continue;
		...
		} else if (command.startsWith("admin_send_home")) {
			StringTokenizer st = new StringTokenizer(command);
			if (st.countTokens() > 1) {
				st.nextToken();
				String plyr = st.nextToken();
