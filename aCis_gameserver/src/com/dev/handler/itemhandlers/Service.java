...
public class Service implements IItemHandler
{
	...
	public void useItem(Player player, ItemInstance item)
	{
		...
		if (player.isCastingNow() || player.isOutOfControl() || player.isInOlympiadMode() || player.isInObserverMode() || player.isInJail() || player.isInDuel())
		{
		...
		switch (item.getServiceId())
		{
			...
			case 3:
				...
				player.destroyItem("Consume", item.getObjectId(), 1, null, true);
				player.sendPacket(new CreatureSay(0, 17, "Service", "Congratulations, your recommend score has been increased."));
			...
			case 4:
				player.destroyItem("Consume", item.getObjectId(), 1, null, true);
				player.sendPacket(new CreatureSay(0, 17, "Service", "Congratulations, your gender has been changed,you will be disconected in 3 seconds."));
				...
				else
				{
					...
					for (IntIntHolder itemAIO : Config.LIST_AIO_ITEMS)
					{
						if (itemAIO.getId() > 0)
						{
							...
							player.getInventory().equipItemAndRecord(player.getInventory().getItemByItemId(itemAIO.getId()));
						}
					}
				}
			...
			case 7:
				...
				player.destroyItem("Consume", item.getObjectId(), 1, null, true);
				player.sendPacket(new CreatureSay(0, 17, "Service", "Congratulations, you are now a noble, you are granted with noblesse status and noblesse skills."));
				player.sendPacket(new ExShowScreenMessage("You are now a noble, you are granted with noblesse status", 10000));
