	...
	public synchronized void onMobKill(DungeonMob mob) {
		...
		if (mobs.isEmpty()) {
			...
			if (currentStage == null) // No more stages
			{
				...
				broadcastScreenMessage("You have completed stage " + (currentStage.getOrder() - 1)
						+ "! Next stage begins in 10 seconds.", 5);
...
//			PlayerMemo.setVar(player, "dungeon_atleast1time", "true", -1);
			...
			for (Entry<Integer, Integer> itemId : template.getRewards().entrySet()) {
				...
				System.out.println("ItemId Key: " + itemId.getKey());
				System.out.println("ItemId Value: " + itemId.getValue());
				System.out.println("Recompensa da Dungeon");
		...
		if (!template.getRewardHtm().equals("NULL")) {
			...
			System.out.println("HTML Exibido: " + template.getRewardHtm());
			for (Player player : players)
	...
	private void teleToTown() {
		for (Player player : players) {
			...
			System.out.println("Removendo player da dungeon");
			System.out.println("PlayerID: " + player.getObjectId());

			for (Integer participants : DungeonManager.getInstance().getDungeonParticipants())
				System.out.println("Participante: " + participants);
...
//			DungeonManager.getInstance().getDungeonParticipants().remove(player.getObjectId());
	...
	private void cancelDungeon() {
		...
		broadcastScreenMessage("You have failed to complete the dungeon. You will be teleported back in 10 seconds.",
				5);

		System.out.println("Cancelando timers");
		if (nextTask != null)
			nextTask.cancel(true);
		if (timerTask != null)
			timerTask.cancel(true);
	...
	private void beginStage() {
		...
		broadcastScreenMessage(
				"You have " + currentStage.getMinutes() + " minutes to finish stage " + currentStage.getOrder() + "!",
				5);
	...
	private void getNextStage() {
		currentStage = currentStage == null ? template.getStages().get(1)
				: template.getStages().get(currentStage.getOrder() + 1);
	}

	private void broadcastTimer() {
		int secondsLeft = (int) (((stageBeginTime + (1000 * 60 * currentStage.getMinutes()))
				- System.currentTimeMillis()) / 1000);
		int minutes = secondsLeft / 60;
		int seconds = secondsLeft % 60;
		ExShowScreenMessage packet = new ExShowScreenMessage(String.format("%02d:%02d", minutes, seconds), 1010,
				SMPOS.BOTTOM_RIGHT, false);
		for (Player player : players)
